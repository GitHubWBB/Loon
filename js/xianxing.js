...
const reg_xxtoday = /<div class="days twenty-six font">今日限行<\/div>[\s\S]*?<div class="date twenty-two">(.+?)<\/div>[\s\S]*?<div class="rule thirty-four font-nine">(.+?)<\/div>/;

const reg_xxtomorrow = /<div class="days twenty-six font">明日限行<\/div>[\s\S]*?<div class="date twenty-two">(.+?)<\/div>[\s\S]*?<div class="rule thirty-four font-nine">(.+?)<\/div>/;

const reg_xxdate = /<div class="three twenty-eight">限行时间<\/div>[\s\S]*?<div class="limit-local xianxing-content">/;
...
function a (arr) {
	for (let i=0; i < arr.length; i++) {
		arr[i] = arr[i].replace(/<div class="three twenty-eight">(.+)/,'$1: ↓↓↓').replace(/<\/div>\s+<\/div>[\s\S]+?<div class=".+xianxing-content">/,'').trim();
	}
	return arr
};//for
